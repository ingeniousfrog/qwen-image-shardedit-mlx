#!/usr/bin/env python3
"""A/B: residency-window image attention output projection as q6 vs dense bf16."""
...
from shardedit_mlx.dense_img_attn_out_window import prepare_dense_img_attn_out_window
...
@dataclass(frozen=True)
class DenseImgAttnOutWindowResult:
    mlx_version: str
    platform: str
    model: str
...
def time_path(
    *,
    ...
    def once() -> Any:
        ...
        if dense:
            handles = prepare_dense_img_attn_out_window(blocks)
...
def decide_verdict(
    *,
    ...
    all_finite_value: bool,
    speedup_threshold: float,
    error_tolerance: float,
) -> tuple[str, str]:
    if not all_finite_value:
        return "dense_img_attn_out_invalid", "dense output produced non-finite values"
    if max_abs_error_value > error_tolerance:
        return (
            "dense_img_attn_out_error",
            f"dense max_abs_error {max_abs_error_value:.6g} exceeds tolerance {error_tolerance:.6g}",
        )
    speedup = relative_speedup(q6_median, dense_median)
    if speedup >= 1.0 + speedup_threshold:
        return (
            "dense_img_attn_out_helps",
            f"dense attention output projection is {speedup:.3f}x faster than q6",
        )
    return (
        "dense_img_attn_out_not_enough",
        f"dense speedup {speedup:.3f}x is below {1.0 + speedup_threshold:.3f}x gate",
    )


def run_benchmark(args: argparse.Namespace) -> DenseImgAttnOutWindowResult:
    ...
    dense_output, dense_path = time_path(
        name="dense_img_attn_out_window",
        layout=layout,
        block_indices=block_indices,
        inputs=inputs,
        dense=True,
        ...
        **{
            **asdict(dense_path),
            "relative_to_q6": relative_speedup(
                q6_path.median_seconds,
                dense_path.median_seconds,
            ),
    ...
    verdict, reason = decide_verdict(
        ...
        max_abs_error_value=error,
        all_finite_value=dense_path.all_finite,
        speedup_threshold=args.speedup_threshold,
        error_tolerance=args.error_tolerance,
    )
    return DenseImgAttnOutWindowResult(
